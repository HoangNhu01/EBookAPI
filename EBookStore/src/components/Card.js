...
const Card = ({ product }) => {
    ...
    return (
        <div className="product flex flex-col gap-2 bg-white shadow-md rounded-xl overflow-hidden hover:shadow-2xl duration-300">
            <div className="product-img" style={{ margin: 'auto' }}>
                <img
                    src={'data:image/jpeg;base64,' + product.thumbnailImage}
                    alt={product.name}
                />
            </div>
            <div className="product-content flex flex-col gap-2 pb-5 px-5">
                ...
                <div className="flex justify-between items-center">
                    <div>
                        <div className="md:text-xl text-lg font-medium text-rose-500">
                            {/* //This fucntion for formate price in USD currency */}
                            {currencyFormatter(product.price)}
                        </div>

                        <Link  to={`/products/${product.id}`} style={{color: '#d6d65e'}}>View Detail</Link>
                    </div>
                    <button
